...
def word_count_messages(messages: list[dict]) -> int:
    return sum(len(message_text(m).split()) for m in messages)


def flatten_messages(messages: list[dict], system: str | None = None) -> str:
    parts = [message_text(m) for m in messages]
    if system:
        parts.insert(0, system)
    return " ".join(parts)


# Approximate $/1M-token input rates for common model families (input side).
# Heuristic prefix match, used only for indicative USD figures in telemetry;
# billable truth stays with the provider invoice.
